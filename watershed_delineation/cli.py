...
from watershed_core import delineate_watershed
...
def main():
    ...
    parser.add_argument(
        "dem_file",
        type=str,
        help="Path to the input DEM file (.tif)."
    )
    parser.add_argument(
        "pour_lon",
        type=float,
        help="Longitude of the pour point (e.g., -122.4194)."
    )
    parser.add_argument(
        "pour_lat",
        type=float,
        help="Latitude of the pour point (e.g., 37.7749)."
    )
    parser.add_argument(
        "-o", "--output",
        type=str,
    ...
    parser.add_argument(
        "-n", "--name",
        type=str,
        help="Name for the output shapefile. Defaults to 'watershed_lon_lat'."
    )
    parser.add_argument(
        "--export-lfp",
        action="store_true",
        help="Export the longest flow path as a separate shapefile."
    )
